...
const WeatherData = () => {
  ...
  console.log(answer);
  const gameAgain = () => {
    let availableCities = cityArray.filter(
      (city) => !currentArray.includes(city)
    );
  ...
  const handleCheck = () => {
    ...
    setCurrentArray((prevArray) => {
      ...
      if (newArray.length >= 5) {
        setCurrentCity("");
        const correctAnswers = newArray.filter(
          (_, index) => results[index]?.answer === true
        ).length;
        if (correctAnswers >= 3) {
          setGameResult(true);
        } else {
